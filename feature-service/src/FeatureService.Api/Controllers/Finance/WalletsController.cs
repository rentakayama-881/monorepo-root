...
public class WalletsController : ApiControllerBase
{
    ...
    /// <summary>
    /// Deduct wallet balance for AI chat usage
    /// </summary>
    [HttpPost("ai-chat/spend")]
    [ProducesResponseType(typeof(AiChatSpendResponse), 200)]
    [ProducesResponseType(typeof(ApiErrorResponse), 400)]
    [ProducesResponseType(typeof(ApiErrorResponse), 401)]
    public async Task<IActionResult> SpendForAiChat([FromBody] AiChatSpendRequest request)
    {
        var user = _userContextAccessor.GetCurrentUser();
        if (user == null)
        {
        ...
        if (request.Amount <= 0)
        {
            return ApiBadRequest("Amount harus lebih dari 0");
        }

        if (string.IsNullOrEmpty(request.Model))
        {
            return ApiBadRequest("Model tidak boleh kosong");
        }
        ...
        try
        {
            ...
            var (success, error, _) = await _walletService.DeductBalanceAsync(
                ...
                Models.Entities.TransactionType.AiChat,
                request.SessionId,
                "ai_chat"
            );
            ...
            if (!success)
            {
                return ApiBadRequest(error ?? "Gagal memproses pembayaran");
            }

            var updatedWallet = await _walletService.GetOrCreateWalletAsync(user.UserId);

            _logger.LogInformation("AI Chat spend: User {UserId} spent {Amount} for model {Model}", 
                user.UserId, request.Amount, request.Model);

            return Ok(new AiChatSpendResponse(
                true,
                updatedWallet.Balance,
                $"Berhasil menggunakan Rp{request.Amount:N0} untuk AI Chat"
            ));
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error processing AI chat spend for user {UserId}", user.UserId);
            return ApiInternalError("Gagal memproses pembayaran AI Chat");
        }
    }
}
