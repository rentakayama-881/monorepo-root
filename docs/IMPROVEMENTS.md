...
#### 2.2.1 Vector Search dengan pgvector

```sql
-- Enable extension
CREATE EXTENSION vector;

-- Add embedding column to threads
ALTER TABLE threads ADD COLUMN embedding vector(1536);

-- Create index for fast similarity search
CREATE INDEX ON threads USING ivfflat (embedding vector_cosine_ops);
```

```go
// services/semantic_search.go
func (s *SearchService) SemanticSearch(query string, limit int) ([]*Thread, error) {
    // Get embedding from OpenAI
    embedding, _ := s.openai.CreateEmbedding(query)
    
    // Vector similarity search
    threads, _ := s.db.Thread.Query().
...
#### 2.2.2 Auto-Moderation dengan AI

```go
// services/moderation_ai.go
type ModerationService struct {
    openai *openai.Client
}

func (m *ModerationService) CheckContent(content string) (*ModerationResult, error) {
    resp, _ := m.openai.CreateModeration(ctx, openai.ModerationRequest{
        Input: content,
    })
...
#### 2.2.3 Smart Summarization

```go
// Auto-generate thread summaries
func (s *ThreadService) GenerateSummary(threadID int) (string, error) {
    thread, _ := s.db.Thread.Get(ctx, threadID)
    
    resp, _ := s.openai.CreateChatCompletion(ctx, openai.ChatCompletionRequest{
        Model: "gpt-4-turbo",
        Messages: []openai.ChatCompletionMessage{
            {Role: "system", Content: "Summarize the following thread in 2-3 sentences in Indonesian:"},
