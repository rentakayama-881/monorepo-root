...
## As-Is (verified)
- Backend Go menggunakan Gin dengan JWT middleware, OAuth GitHub, dan route REST yang didefinisikan di `backend/main.go` (mis. `/api/threads/categories`, `/api/threads/latest`, `/api/threads/:id`).
...
- JWT disimpan di `localStorage` (mis. dicek di `Header`, `ProfileSidebar`) dan dipakai untuk memanggil endpoint yang memerlukan auth.
