...
export default async function CategoryGrid() {
  ...
  const href = primaryCategory ? `/category/${primaryCategory.slug}` : "/validation-cases";
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 stagger-children">
          {categories.map((cat) => (
            <Link key={cat.slug} href={`/category/${cat.slug}`} className="block h-full group">
              <Card className="flex h-full items-center justify-center p-6 text-center transition-all hover:border-primary hover:bg-secondary/40">
