...
type AdminQuery struct {
	...
	withGrantedBadges *UserBadgeQuery
...
// QueryGrantedBadges chains the current query on the "granted_badges" edge.
func (_q *AdminQuery) QueryGrantedBadges() *UserBadgeQuery {
...
func (_q *AdminQuery) Clone() *AdminQuery {
	...
	return &AdminQuery{
		...
		withGrantedBadges: _q.withGrantedBadges.Clone(),
...
// WithGrantedBadges tells the query-builder to eager-load the nodes that are connected to
// the "granted_badges" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *AdminQuery) WithGrantedBadges(opts ...func(*UserBadgeQuery)) *AdminQuery {
	query := (&UserBadgeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withGrantedBadges = query
	return _q
}
...
func (_q *AdminQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Admin, error) {
	...
	if query := _q.withGrantedBadges; query != nil {
		if err := _q.loadGrantedBadges(ctx, query, nodes,
			func(n *Admin) { n.Edges.GrantedBadges = []*UserBadge{} },
			func(n *Admin, e *UserBadge) { n.Edges.GrantedBadges = append(n.Edges.GrantedBadges, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AdminQuery) loadGrantedBadges(ctx context.Context, query *UserBadgeQuery, nodes []*Admin, init func(*Admin), assign func(*Admin, *UserBadge)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userbadge.FieldGrantedBy)
	}
	query.Where(predicate.UserBadge(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(admin.GrantedBadgesColumn), fks...))
	}))
