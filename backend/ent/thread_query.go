...
func (_q *ThreadQuery) QueryUser() *UserQuery {
...
func (_q *ThreadQuery) QueryCategory() *CategoryQuery {
...
func (_q *ThreadQuery) QueryTags() *TagQuery {
...
func (_q *ThreadQuery) WithUser(opts ...func(*UserQuery)) *ThreadQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *ThreadQuery) WithCategory(opts ...func(*CategoryQuery)) *ThreadQuery {
	query := (&CategoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ThreadQuery) WithTags(opts ...func(*TagQuery)) *ThreadQuery {
	query := (&TagClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ThreadQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Thread, error) {
	...
	if query := _q.withUser; query != nil {
		if err := _q.loadUser(ctx, query, nodes, nil,
			func(n *Thread, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withCategory; query != nil {
		if err := _q.loadCategory(ctx, query, nodes, nil,
			func(n *Thread, e *Category) { n.Edges.Category = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *ThreadQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Thread, init func(*Thread), assign func(*Thread, *User)) error {
...
func (_q *ThreadQuery) loadCategory(ctx context.Context, query *CategoryQuery, nodes []*Thread, init func(*Thread), assign func(*Thread, *Category)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Thread)
	for i := range nodes {
...
func (_q *ThreadQuery) loadTags(ctx context.Context, query *TagQuery, nodes []*Thread, init func(*Thread), assign func(*Thread, *Tag)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(tag.FieldID), joinT.C(thread.TagsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(thread.TagsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(thread.TagsPrimaryKey[1]))
...
func (_q *ThreadQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withCategory != nil {
			_spec.Node.AddColumnOnce(thread.FieldCategoryID)
		}
	}
